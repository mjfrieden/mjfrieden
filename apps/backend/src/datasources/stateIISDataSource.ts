...
export class StateIISDataSource implements ImmunizationDataSource {
  id = 'state_iis';
  displayName = 'State Immunization Registry';

  async connect(_userId: string): Promise<void> {
    // Placeholder for future HL7/FHIR connection
    return;
  }

  async fetchImmunizations(_userId: string): Promise<FhirImmunization[]> {
    const file = path.join(__dirname, '..', 'data', 'state-iis.json');
